...
HHNTUP ?= ntuples_hh/prod_v5_sys/hhskim
...
.PHONY: bdt-score
bdt-score:
	find $(HHNTUP_RUNNING) -name "*.root" | grep "bdtscore." | xargs rm -f
	nohup find $(HHNTUP_RUNNING) -name "*\.root" | grep "tautauhh" | sort| xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_signal.log &
	nohup find $(HHNTUP_RUNNING) -name "*\.root" | grep "embed" | sort| xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_embeded.log &
	nohup find $(HHNTUP_RUNNING) -name "*\.root" | grep "JetTauEtmiss\." | sort| xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_data.log &
	nohup find $(HHNTUP_RUNNING) -name "*\.root" | grep -v "JetTauEtmiss" | grep -v "tautauhh" | grep -v "embed" | sort | xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_mc.log &

.PHONY: bdt-score-signal
bdt-score-signal:
#	find $(HHNTUP_RUNNING) -name "*\.root" | grep "tautauhh" | grep "bdtscore." | xargs rm -f
	find $(HHNTUP_RUNNING) -name "*\.root" | grep "tautauhh" | sort
	find $(HHNTUP_RUNNING) -name "*\.root" | grep "tautauhh" | sort| xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_signal.log &

.PHONY: bdt-score-embed
bdt-score-embed:
#	find $(HHNTUP_RUNNING) -name "*\.root" | grep "embed" | grep "bdtscore." | xargs rm -f
	find $(HHNTUP_RUNNING) -name "*\.root" | grep "embed" | sort
	find $(HHNTUP_RUNNING) -name "*\.root" | grep "embed" | sort | xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_embed.log &

.PHONY: bdt-score-data
bdt-score-data:
#	find $(HHNTUP_RUNNING) -name "*\.root" | grep "JetTauEtmiss" | grep "bdtscore." | xargs rm -f
	find $(HHNTUP_RUNNING) -name "*\.root" | grep "JetTauEtmiss\." | sort
	find $(HHNTUP_RUNNING) -name "*\.root" | grep "JetTauEtmiss\." | sort | xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_data.log &

.PHONY: bdt-score-mc
bdt-score-mc:
	find $(HHNTUP_RUNNING) -name "*\.root" | grep -v "JetTauEtmiss" | grep -v "tautauhh" | grep -v "embed" | grep "bdtscore." | xargs rm -f
	find $(HHNTUP_RUNNING) -name "*\.root" | grep -v "JetTauEtmiss" | grep -v "tautauhh" | grep -v "embed" | sort
	find $(HHNTUP_RUNNING) -name "*\.root" | grep -v "JetTauEtmiss" | grep -v "tautauhh" | grep -v "embed" | xargs -n 1 ./bdt-score >& $(HHNTUP_RUNNING)/bdtscore_mc.log &
...
.PHONY: norms_cp
norms_cp:
	@for category in presel presel_pipi  presel_rhorho  presel_pirho  presel_rhopi; do \
		for model in OS_NONISOL nOS nOS_ISOL nOS_NONISOL SS SS_ISOL SS_NONISOL NONISOL; do \
			nohup ./norm --norm-category $${category} --fakes-region $${model} --year 2012 & \
			nohup ./norm --norm-category $${category} --no-embedding --fakes-region $${model} --year 2012 & \
		done; \
		nohup ./norm --norm-category $${category} --fakes-region SS_NTRK --target-region OS_NTRK --year 2012 & \
		nohup ./norm --norm-category $${category} --no-embedding --fakes-region SS_NTRK --target-region OS_NTRK --year 2012 & \
		nohup ./norm --norm-category $${category} --fakes-region SS --target-region OS --year 2012 & \
		nohup ./norm --norm-category $${category} --no-embedding --fakes-region SS --target-region OS --year 2012 & \
	done

.PHONY: norms_presel
norms_presel:
	@for category in presel; do \
		for model in OS_NONISOL nOS nOS_ISOL nOS_NONISOL SS SS_ISOL SS_NONISOL NONISOL; do \
			nohup ./norm --norm-category $${category} --fakes-region $${model} --year 2012 & \
			nohup ./norm --norm-category $${category} --no-embedding --fakes-region $${model} --year 2012 & \
		done; \
...
.PHONY: norms_presel_pipi
norms_presel_pipi:
	@for category in presel_pipi; do \
		for model in OS_NONISOL nOS nOS_ISOL nOS_NONISOL SS SS_ISOL SS_NONISOL NONISOL; do \
			nohup ./norm --norm-category $${category} --fakes-region $${model} --year 2012 & \
			nohup ./norm --norm-category $${category} --no-embedding --fakes-region $${model} --year 2012 & \
		done; \
...
.PHONY: norms_presel_pirho
norms_presel_pirho:
	@for category in presel_pirho; do \
		for model in OS_NONISOL nOS nOS_ISOL nOS_NONISOL SS SS_ISOL SS_NONISOL NONISOL; do \
			nohup ./norm --norm-category $${category} --fakes-region $${model} --year 2012 & \
			nohup ./norm --norm-category $${category} --no-embedding --fakes-region $${model} --year 2012 & \
		done; \
...
norms_presel_rhopi:
	@for category in presel_rhopi; do \
		for model in OS_NONISOL nOS nOS_ISOL nOS_NONISOL SS SS_ISOL SS_NONISOL NONISOL; do \
			nohup ./norm --norm-category $${category} --fakes-region $${model} --year 2012 & \
			nohup ./norm --norm-category $${category} --no-embedding --fakes-region $${model} --year 2012 & \
		done; \
...
norms_presel_rhorho:
	@for category in presel_rhorho; do \
		for model in OS_NONISOL nOS nOS_ISOL nOS_NONISOL SS SS_ISOL SS_NONISOL NONISOL; do \
			nohup ./norm --norm-category $${category} --fakes-region $${model} --year 2012 & \
			nohup ./norm --norm-category $${category} --no-embedding --fakes-region $${model} --year 2012 & \
		done; \
