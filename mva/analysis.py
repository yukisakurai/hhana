...
class Analysis(object):
    ...
    def __init__(self, year,
        ...
        self.higgs_odd_125 = samples.Higgs(
            year=year,
            mass=125,
            systematics=systematics,
            linecolor='blue',
        ...
        self.signals_odd = self.get_signals_odd(125)
    ...
    def get_signals_odd(self, mass=125, mode=None, scale_125=False):
        signals = []
        if not isinstance(mass, list):
            mass = [mass]
        if scale_125:
            ...
            for m in mass:
                if m != 125 and scale_125:
                    curr_events = samples.Higgs(
                        ...
                        scale=self.mu,
                        ggf_weight=self.ggf_weight).events()[1].value
                    log.warning("SCALING CP ODD SIGNAL TO CP EVEN SIGNAL 125")
                    sf = events_125 / curr_events
                    log.info(str(sf))
                for mode in samples.Higgs.MODES:
                    s = samples.Higgs(
    ...
    def get_channel(self, hist_template, expr_or_clf, category, region,
        ...
        if include_signal:
            ...
            if include_signal_odd:
                samples += self.get_signals_odd(mass, mode)
    ...
    def get_channel_array(self, vars,
        ...
        if include_signal:
            ...
            if include_signal_odd:
                samples += self.get_signals_odd(mass, mode, scale_125=scale_125)
        ...
        for s in samples:
            ...
            # ######## CP ODD NORMALIZATION ###############
            if 'Signal_Z' in s.name:
                for key in vars.keys():
                    signal_z_even_sum[key] = field_sample[key].hist.GetSum()
                    field_sample[key].AddNormFactor('Nev_Signal_Z_125_' + category.name,signal_z_even_sum[key],0.0,100.0)
            elif 'Signal_W' in s.name:
                for key in vars.keys():
                    signal_w_even_sum[key] = field_sample[key].hist.GetSum()
                    field_sample[key].AddNormFactor('Nev_Signal_W_125_' + category.name,signal_w_even_sum[key],0.0,100.0)
            elif 'Signal_gg' in s.name:
                for key in vars.keys():
                    signal_gg_even_sum[key] = field_sample[key].hist.GetSum()
                    field_sample[key].AddNormFactor('Nev_Signal_gg_125_' + category.name,signal_gg_even_sum[key],0.0,100.0)
            elif 'Signal_VBF' in s.name:
                for key in vars.keys():
                    signal_vbf_even_sum[key] = field_sample[key].hist.GetSum()
                    field_sample[key].AddNormFactor('Nev_Signal_VBF_125_' + category.name,signal_vbf_even_sum[key],0.0,100.0)
            elif 'Signal' in s.name and not 'Odd' in s.name:
                for key in vars.keys():
                    signal_even_sum[key] = field_sample[key].hist.GetSum()
                    field_sample[key].AddNormFactor('Nev_Signal_125_' + category.name,signal_even_sum[key],0.0,100.0)

            if 'Signal_Odd_Z' in s.name:
                for key in vars.keys():
                    hist_sum = field_sample[key].hist.GetSum()
                    field_sample[key].hist.Scale(signal_z_even_sum[key]/hist_sum)
                    field_sample[key].AddNormFactor('Nev_Signal_Odd_Z_125_' + category.name,signal_z_even_sum[key],0.0,100.0)
            elif 'Signal_Odd_W' in s.name:
                for key in vars.keys():
                    hist_sum = field_sample[key].hist.GetSum()
                    field_sample[key].hist.Scale(signal_w_even_sum[key]/hist_sum)
                    field_sample[key].AddNormFactor('Nev_Signal_Odd_W_125_' + category.name,signal_w_even_sum[key],0.0,100.0)
            elif 'Signal_Odd_gg' in s.name:
                for key in vars.keys():
                    hist_sum = field_sample[key].hist.GetSum()
                    field_sample[key].hist.Scale(signal_gg_even_sum[key]/hist_sum)
                    field_sample[key].AddNormFactor('Nev_Signal_Odd_gg_125_' + category.name,signal_gg_even_sum[key],0.0,100.0)
            elif 'Signal_Odd_VBF' in s.name:
                for key in vars.keys():
                    hist_sum = field_sample[key].hist.GetSum()
                    field_sample[key].hist.Scale(signal_vbf_even_sum[key]/hist_sum)
                    field_sample[key].AddNormFactor('Nev_Signal_Odd_VBF_125_' + category.name,signal_vbf_even_sum[key],0.0,100.0)
            elif 'Signal_Odd' in s.name:
                for key in vars.keys():
                    hist_sum = field_sample[key].hist.GetSum()
                    field_sample[key].hist.Scale(signal_even_sum[key]/hist_sum)
                    field_sample[key].AddNormFactor('Nev_Signal_Odd_125_' + category.name,signal_even_sum[key],0.0,100.0)
    ...
    def get_scores(self, clf, category, region, cuts=None,
        ...
        if masses is not None:
            for mass in masses:
                # signal scores
                sigs_odd = self.get_signals_odd(mass=mass, mode=mode)
                sig_odd_scores = []
                for sig_odd in sigs_odd:
                    scores_dict = sig_odd.scores(
                        clf,
                        category=category,
                        region=region,
                        cuts=cuts,
                        systematics=systematics,
                        systematics_components=sig_odd.systematics_components())
                    ...
                    sig_odd_scores.append((sig_odd, scores_dict))
                all_sig_odd_scores[mass] = sig_odd_scores
        ...
        return Scores(
            ...
            all_sig_odd_scores=all_sig_odd_scores,
            min_score=min_score,
            max_score=max_score)
    ...
    def clf_channels(self, clf,
        ...
        all_sig_odd_scores = scores_obj.all_sig_odd_scores
...
#        data_sample = None
        ...
        # create signal HistFactory samples
        sig_odd_samples = []
        for s, scores in all_sig_odd_scores[mass]:
            hist_template = binning.Clone(
                title=s.label,
                **s.hist_decor)
            sample = s.get_histfactory_sample(
    ...
    def make_var_channels(self, hist_template, expr, categories, region,
                          ...
                          systematics=False, normalize=True, uniform=False):
